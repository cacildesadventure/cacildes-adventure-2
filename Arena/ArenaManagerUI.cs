...
namespace AF.Arena
{
    public class ArenaManagerUI : MonoBehaviour
    {
        ...
        {
            round.text = $"{LocalizationSettings.StringDatabase.GetLocalizedString("Glossary", "Round")} {currentRound} / {maxRounds}";
            root.style.display = DisplayStyle.Flex;
        }

        public void UpdateWaitingForNextRound(int timeBeforeNextRound)
        {
            round.text = $"{LocalizationSettings.StringDatabase.GetLocalizedString("Glossary", "Next round will begin in")} {timeBeforeNextRound}";
            root.style.display = DisplayStyle.Flex;
        }
