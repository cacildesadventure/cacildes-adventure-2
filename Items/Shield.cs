    ...
    public class Shield : Item
    {
        ...
        public string GetFormattedStatusResistances()
        {
            ...
            foreach (var resistance in statusEffectBlockResistances)
            {
                if (resistance != null)
                {
                    result += $"%{100 - (resistance.absorption * 100)} {resistance.statusEffect.GetName()} {LocalizationSettings.StringDatabase.GetLocalizedString("Glossary", "Absorption")}\n";
                }
            }
        ...
        public string GetFormattedStatusCancellationRates()
        {
            ...
            foreach (var resistance in statusEffectCancellationRates)
            {
                if (resistance != null)
                {
                    result += $"-{resistance.amountToCancelPerSecond} {resistance.statusEffect.GetName()} {LocalizationSettings.StringDatabase.GetLocalizedString("Glossary", "Inflicted Per Second")}\n";
                }
            }
        ...
        public string GetFormattedStatusAttacks()
        {
            ...
            foreach (var resistance in damageDealtToEnemiesUponBlocking.statusEffects)
            {
                if (resistance != null)
                {
                    result += $"+{resistance.amountPerHit} {resistance.statusEffect.name} {LocalizationSettings.StringDatabase.GetLocalizedString("Glossary", "inflicted on enemy attacking shield")}\n";
                }
            }
